...
CI runs `scripts/verify-release-config.sh` to check that the checked-in HealthKit entitlement, CloudKit service, iCloud container, Google OAuth callback scheme, remote-notification background mode, privacy manifest, Xcode entitlements path, and SwiftData private CloudKit container stay aligned.
