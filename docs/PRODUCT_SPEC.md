...
CI also verifies the checked-in release configuration for the HealthKit entitlement, private CloudKit container, Google OAuth callback scheme, Xcode entitlements path, SwiftData CloudKit container ID, HealthKit usage strings, CloudKit remote-notification background mode, and the privacy manifest.
